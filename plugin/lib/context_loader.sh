#!/usr/bin/env bash
# context_loader.sh -- Context projection builder integration (plugin version)
...
source "$_LIB_DIR/projection_check.sh"
...
build_context_if_needed() {
  ...
  # Try calling the project CLI from the plugin scripts directory first
  local plugin_project_bin
  plugin_project_bin="$(cd "$(dirname "${BASH_SOURCE[0]}")/../scripts" && pwd)/project"
  if [[ -x "$plugin_project_bin" ]]; then
    "$plugin_project_bin" context "$project_id" "$session_id" 2>/dev/null || true
    return 0
  fi

  # Fallback: try the installed project CLI
